...
class Process:
    def __init__(self, obj_file, target_faces, export_name):
        ...
        export_name="./simplified/"+export_name[2:-4]+"_simplified.obj"
    ...
    def simplify(self, mesh, target_faces):
        ...
        mod1 = mesh.modifiers.new(name='Triangluate', type='TRIANGULATE')
        ...
        if nfaces < 750:
            self.subsurf(mesh)
            nfaces = len(mesh.data.polygons)
        nedges = len(mesh.data.polygons)
        
        ratio = target_faces / float(nedges)
        ...
        # auto remove if operation was unsuccessful
        if (len(mesh.data.edges) >= target_faces):
            os.remove(obj_file)
            cancel_export = True
