...
class Point_Transformer(nn.Module):
    def __init__(self, config):
        ...
        self.radius_cnn = create_rFF3d(
            self.radius_ch, self.radius_max_points + 1, self.input_dim
        )
        ...
        self.last_layer = PTransformerDecoderLayer(
            self.d_model, nhead=8, last_dim=out_dim
        )
        self.custom_decoder = PTransformerDecoder(
            self.decoder_layer, 1, self.last_layer
        )
    ...
    def forward(self, input):
        ...
        radius_grouped = torch.cat(
            [
                x_local_sorted.transpose(2, 1),
                radius_grouped,
                x_local_scores.transpose(2, 1),
            ],
            dim=1,
        )
