                    ...
                    for batch_size in [4, 8, 32]:
                        for dataset_type in [
                            ...
                            with open(f"{data}_gridsearch_{rand_id}.sh", "w") as w:
                                ...
                                w.write(
                                    f"python scripts/train_3dcnn.py --kernel_size {kernel_size}  --epochs {epoch} --activation_fn {activation_fn} --learning_rate {learning_rate} --batch_size {batch_size} --num {data} --type {dataset_type}\n"
                                )
